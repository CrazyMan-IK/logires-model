	...
	*/
	...
	private string _result = "";
	    ...
	    public Ellipse(IReadOnlyVector2<float> position, IReadOnlyVector2<float> size, bool isFilled, Color color)
	    {
	        ...
	        {
	            var rad = MathExtensions.Lerp(0, Math.PI * 2, i);
	            var curX = position.x + size.x + Math.sin(rad) * size.x;
	            var curY = position.y + size.y + Math.cos(rad) * -size.y;
	    ...
	    {
	    	var dx = MathF.Pow(point.X - Position.X - Size.X * 0.5, 2) / MathF.Pow(Size.X * 0.5 - 0.1, 2);
	    	var dy = MathF.Pow(point.Y - Position.Y - Size.Y * 0.5, 2) / MathF.Pow(Size.Y * 0.5 - 0.1, 2);
	    	return dx + dy <= 1;
	    }
}
